...
*
* NOTE: Based on Epic Games Lyra's LoadingScreenManager, though we modified it to suit our needs.
*/
...
void ULoadingScreenManager::ShowLoadingScreen()
{
	...
	else
	{
		...
		OnLoadingScreenVisibilityChangedDynamic.Broadcast(/*bIsVisible=*/ true);

		// Create the loading screen widget
		if (!LoadingScreenInstance.Get())
		{
			const TSubclassOf<UUserWidget> LoadingScreenWidgetClass = Settings->ActivatableLoadingScreenWidget.TryLoadClass<UUserWidget>();
			if (UUserWidget* UserWidget = UUserWidget::CreateWidgetInstance(*LocalGameInstance, LoadingScreenWidgetClass, NAME_None))
			{
				LoadingScreenInstance = Cast<UCommonActivatableWidget>(UserWidget);

				// Add to the viewport at a high ZOrder to make sure it is on top of most things
				UGameViewportClient* GameViewportClient = LocalGameInstance->GetGameViewportClient();
				GameViewportClient->AddViewportWidgetContent(LoadingScreenInstance->TakeWidget(), Settings->LoadingScreenZOrder);
			}
			else
			{
				UE_LOG(LogLoadingScreen, Error, TEXT("Failed to load the loading screen widget %s, falling back to placeholder."), *Settings->ActivatableLoadingScreenWidget.ToString());
			}
		}

		if (LoadingScreenInstance.Get())
		{
			LoadingScreenInstance->ActivateWidget();
		}
...
void ULoadingScreenManager::HideLoadingScreen()
{
	...
	else
	{
		...
		OnLoadingScreenVisibilityChangedDynamic.Broadcast(/*bIsVisible=*/ false);
		LoadingScreenHidden.Broadcast();
	}
...
void ULoadingScreenManager::RemoveWidgetFromViewport()
{
	...
	if (LoadingScreenInstance.Get())
	{
		LoadingScreenInstance->DeactivateWidget();
	}
}
